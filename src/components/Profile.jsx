...
const AdminProfile = () => {
  ...
  const handleChange = (e) => {
    ...
    setIsEditing(true); // Mark as editing when any field changes
  };

  // Function to handle the form submission (Save Changes)
  const handleSubmit = (e) => {
    e.preventDefault();
    // In a real application, you would send the 'profile' data to a backend API here
    console.log('Saving changes:', profile);
    setIsEditing(false); // Reset editing state after successful save
    // Add logic for API call, success/error messages, etc.
 ...
 const handlePhotoChange = (e) => {
  ...
  const ProfileInputField = ({ label, name, value, onChange, readOnly = false }) => (
      ...
      <input
        ...
        className={`
          ...
          ${readOnly 
            ...
            : 'bg-white border-gray-400 focus:border-[#228B22] focus:ring-1 focus:ring-[#228B22]'
          }
       ...
       {/* Photo Section */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Role (Read-Only) */}
            {/* Role input uses the same styling but is marked readOnly */}
            <ProfileInputField
              label="Role"
              name="role"
              value={profile.role}
          ...
          <div className="mt-8 pt-4">
            <button
              type="submit"
              disabled={!isEditing} // Disable if no changes have been made
              className={`
                px-6 py-3 rounded-lg text-lg font-semibold w-full sm:w-auto
                transition-colors duration-200
                ${isEditing
                  ? 'bg-[green] hover:bg-green-500 text-white shadow-md'
                  : 'bg-green-500 text-gray-100 cursor-not-allowed'
                }
              `}
            >
              Save Changes
